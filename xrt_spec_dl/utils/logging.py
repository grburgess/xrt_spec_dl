...
xrt_spec_dl_dev_log_handler = handlers.TimedRotatingFileHandler(
...
xrt_spec_dl_dev_log_handler.setFormatter(_dev_formatter)
xrt_spec_dl_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

xrt_spec_dl_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

xrt_spec_dl_usr_log_handler.setLevel(logging.INFO)
...
xrt_spec_dl_usr_log_handler.setFormatter(_usr_formatter)
...
xrt_spec_dl_console_log_handler = logging.StreamHandler(sys.stdout)
xrt_spec_dl_console_log_handler.setFormatter(_console_formatter)
xrt_spec_dl_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    xrt_spec_dl_usr_log_handler.addFilter(warning_filter)
    xrt_spec_dl_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    xrt_spec_dl_usr_log_handler.removeFilter(warning_filter)
    xrt_spec_dl_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    xrt_spec_dl_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(xrt_spec_dl_dev_log_handler)

    log.addHandler(xrt_spec_dl_console_log_handler)

    log.addHandler(xrt_spec_dl_usr_log_handler)
